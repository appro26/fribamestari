...
window.getHoleCellHTML = function(hData, hIndex, isActive, isHistory) {
    ...
    if (hIndex >= 2) {
        ...
        if (worstPlayers.length === 1) {
            ...
            dText = "Tasapeli pohjalla! " + rawInsult.replace(/\[Pelaaja\],\s*/g, '').replace(/\[Pelaaja\]\s*/g, '').replace(/\[Pelaaja\]/g, '').charAt(0).toUpperCase() + rawInsult.slice(1);
        }
...
window.openTargetModal = function(cardId) {
    ...
    if (cost > 0 && me.score < cost) {
        alert(`Ei varaa! Tarvitset ${cost} P pelataksesi tämän Taso ${cardDef.level} kortin.`); return;
    }
        ...
        if (me.score >= pPrice) {
            ...
            if (isRes) {
                ...
                if (nextShopAll[myName]) {
                    ...
                    if (sIdx !== -1) nextShopAll[myName][sIdx] = null; // Jätetään tyhjäksi automaattiin
                }
...
// ==============================================
// VÄLIPALA-AUTOMAATTI (KAUPAN TOIMINNOT)
// ==============================================
window.reserveShopItem = function(idStr) {
    ...
    if (nextShopAll[myName]) {
        ...
        if (sIdx !== -1) nextShopAll[myName][sIdx] = null; // Korvataan nullilla -> Tyhjä kierrejousi UI:hin
    }
...
window.buyShopItem = function(idStr, priceVal, isReservation) {
    ...
    if (isReservation) {
        ...
        if (nextShopAll[myName]) {
            const sIdx = nextShopAll[myName].findIndex(i => i && i.id === idStr);
            if (sIdx !== -1) nextShopAll[myName][sIdx] = null; // Korvataan nullilla
        }
    }
...
window.initNativeCarousel = function() {
    // Tyhjä funktio, jotta ei tule erroria kun showEventCard kutsuu tätä. Karuselli on nyt lukittu.
};
...
window.renderCarousel = function() {
    ...
    window.carouselCards.forEach((cId, i) => {
        ...
        let backHtml = '';
        if (cDef.nextId && cDef.upgradeDesc && window.carouselCurrentMode === 'sell') {
                ...
                <div class="card-3d-inner ${flippedClass}" id="card3d-inner-${i}">
                    <div class="card-face card-front ${typeClass}">
                        <div style="text-align:left; display:flex; flex-direction:column; height:100%; position:relative; z-index:20;">
                            ...
                            ${window.carouselCurrentMode === 'sell' && cDef.nextId ? `<div style="text-align:center; font-weight:900; color:#111; margin-top:auto; padding-bottom:10px;">🔄 KÄÄNNÄ PÄIVITTÄÄKSESI</div>` : ''}
                        </div>
                    </div>
...
window.openCardDetail = function(cId, mode) {
    const me = (allPlayers || []).find(p => p && p.name === myName);
    window.flippedCards = new Set();
    window.carouselCards = [cId]; // Aina vain 1 kortti, ei selaamista
    ...
    // Ei enää Sulje-nappia täällä, koska se luodaan HTML:ssä oikeaan paikkaan
...
// ==============================================
// RADAN HALLINTA LENNOLTA JA ASETUKSET (GM)
// ==============================================
...
window.gmKickPlayer = function(pName) {
    if(confirm(`Haluatko varmasti potkia pelaajan ${pName}?`)) {
...
window.createNewCard = function() {
    ...
    let level = parseInt(el('newCardLevel').value) || 1;

    if(!name || !desc) { alert("Nimi ja kuvaus ovat pakollisia!"); return; }

    let newId = "custom_" + Date.now();
    let family = "custom_fam_" + Date.now();
    let p = (level === 3) ? 15 : ((level === 2) ? 7 : 3);

    let newCardObj = {
        id: newId, family: family, n: `${name} (Taso ${level})`,
        d: desc, level: level, maxLevel: level, upgradeDesc: "",
        nextId: null, type: type, price: p, mech: null
    };

    window.allCards.push(newCardObj);
    
    el('newCardName').value = '';
    el('newCardDesc').value = '';
    el('createCardModal').style.display = 'none';
    window.showAppleToast('Kortti Luotu!', '✅');
