...
window.executeCardPlay = function(targetName) {
    ...
    update(ref(db), { 'gameState/players': window.cleanFirebaseData(nextPlayers), 'gameState/activeHole/playedCards': pCards });
...
window.reserveShopItem = function(idStr) {
    ...
    update(ref(db), { 'gameState/players': window.cleanFirebaseData(nextPlayers), 'gameState/activeHole/shop': nextShopAll });
    ...
    if (mode === 'sell') {
        ...
        if(cDef) {
            ...
            if (cDef.nextId && cDef.upgradeDesc) {
                ...
                btnHtml += `<button class="btn ${canUpg ? 'btn-warning' : 'btn-secondary'}" ${!canUpg ? 'disabled' : ''} style="font-size:1.1rem; padding:15px; margin-top:10px; color:#000;" onclick="event.stopPropagation(); window.upgradeCard('${cId}')">🔼 UPGRADE TASOLLE ${cDef.level + 1} (${upgCost} P)</button>`;
            }
...
// PELAAJAN LISÄÄMINEN (Palautettu)
...
window.gmKickPlayer = function(pName) { if(confirm(`Potki ${pName}?`)) { let nextPlayers = JSON.parse(JSON.stringify(allPlayers)).filter(p => p && p.name !== pName); update(ref(db), {'gameState/players': window.cleanFirebaseData(nextPlayers)}); } };
