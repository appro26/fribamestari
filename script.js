...
//==============================================
// 1. GLOBAALIT FUNKTIOT (KUTSUTAAN HTML-NAPEISTA)
//==============================================
...
window.renderCourseBanner = function() {
    if(!el('courseInfoBanner') || !currentCourse) return;
    ...
    allPlayers.forEach((p, i) => {
        ...
        list.innerHTML += `
            <div class="player-row" style="flex-direction:column; align-items:flex-start; gap:8px;">
                ...
                <div class="gm-score-adjust">
                    <span style="font-size:0.7rem; color:var(--text-muted); width:40px;">Tulos</span>
...
window.nextHole = function() { el('scoreModal').style.display = 'flex'; };
...
window.openTargetModal = function(cardIndex, cardId) {
    ...
    el('targetModal').style.display = 'flex';
};
...
window.executeCardPlay = function(targetName) {
    ...
    el('targetModal').style.display = 'none';
    el('handModal').style.display = 'none';
...
window.changeScore = function(idx, par, delta) {
    ...
    if(val < 1) val = 1;
    input.value = val;
    let display = el(`scoreDisplay_${idx}`);
...
window.submitScores = function() {
    ...
    allPlayers.forEach((p, i) => {
        if(!p) return;
        scores.push({ name: p.name, strokes: parseInt(el(`scoreInput_${i}`).value) || 3 });
    });
