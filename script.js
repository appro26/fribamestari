...
window.showEventCard = function(cId, target, by) {
    ...
    el('cardDetailActionArea').innerHTML = targetStr + `<button class="btn btn-secondary glass-card" style="width:100%; margin-top:10px; padding:16px; font-size:1.1rem; font-weight:bold; color:#fff;" onclick="document.getElementById('cardDetailModal').style.display='none'">❌ SULJE</button>`;
...
window.getHoleCellHTML = function(hData, hIndex, isActive, isHistory) {
    ...
    // HEILUVA KYNÄ ON PALAUTETTU TÄSSÄ!
...
window.renderReceipt = function() {
    ...
    if(el('receipt-full-content')) { 
        ...
        for(let i=0; i<window.gameHistory.length; i++) { 
            let h = window.gameHistory[i]; let par = currentCourse.pars ? (currentCourse.pars[i] || 3) : 3; 
...
window.openTargetModal = function(cardId) {
    ...
    el('targetPlayerList').innerHTML = opponents.map(p => `<button class="btn btn-secondary glass-card" style="width:100%; padding:20px; font-weight:900; font-size:1.3rem; margin-bottom:12px; color:var(--text-main); border:3px solid var(--border);" onclick="window.executeCardPlay('${p.name.replace(/"/g, '&quot;')}')">${p.name}</button>`).join('');
...
window.executeCardPlay = function(targetName) {
    ...
    if(me && me.cards) { 
        ...
        if (card.cost > 0) { me.score -= card.cost; window.logScore(myName, -card.cost, `Pelasi kortin: ${card.def.n}`); window.showAppleToast(`-${card.cost} P`, '💸'); }
    }
...
window.forceDiscard = function(cId) {
    ...
    if(idx !== -1) {
        ...
        window.logScore(myName, sellReward, `Myi kortin`);
    ...
    window.carouselCards.forEach((cId, i) => {
        ...
        let backHtml = `
            <div style="background:#1e293b; border:4px solid #475569; width:100%; height:100%; border-radius:12px; display:flex; flex-direction:column; padding:15px; box-sizing:border-box; color:#fff;">
                ...
                <div style="text-align:center; font-weight:900; font-size:0.75rem; color:#aaa; margin-top:10px;">🔄 KÄÄNNÄ</div>
            </div>`;
                ...
                <div class="card-3d-inner ${flippedClass}" id="card3d-inner-${i}">
                    <div class="card-face card-front ${typeClass}">
                        <div style="text-align:left; display:flex; flex-direction:column; height:100%; position:relative; z-index:20;">
                            ...
                            <div style="text-align:center; font-weight:900; color:#111; margin-top:auto; padding-bottom:10px;">🔄 KÄÄNNÄ (NÄYTÄ TASOT)</div>
                        </div>
                    </div>
...
window.openCardDetail = function(cId, mode) {
    ...
    if (mode === 'sell') {
        ...
        if(cDef) {
            ...
            // NAPIT PINOSSA! Upgrade ylinnä, Pelaa keskellä, Myy alhaalla.
            if (cDef.nextId && cDef.upgradeDesc) {
                let upgCost = cDef.level === 1 ? 3 : 5;
                let canUpg = me.score >= upgCost;
                btnHtml += `<button class="btn ${canUpg ? 'btn-warning' : 'btn-secondary'}" ${!canUpg ? 'disabled' : ''} style="width:100%; font-size:1.1rem; padding:15px; color:#000;" onclick="event.stopPropagation(); window.upgradeCard('${cId}')">🔼 UPGRADE TASOLLE ${cDef.level + 1} (${upgCost} P)</button>`;
            }
            
            btnHtml += `<button class="btn ${canPlay ? 'btn-success' : 'btn-secondary'}" ${!canPlay ? 'disabled' : ''} style="width:100%; font-size:1.2rem; padding:20px; box-shadow:0 4px 15px rgba(16,185,129,0.4);" onclick="document.getElementById('cardDetailModal').style.display='none'; window.openTargetModal('${cDef.id}')">PELAA KORTTI (${playCost} P)</button>`;
            btnHtml += `<button class="btn btn-danger" style="width:100%; font-size:1.1rem; padding:15px;" onclick="window.forceDiscard('${cDef.id}')">♻️ MYY KORTTI (+${sellReward} P)</button>`;
        }
        ...
        if(cDef) {
            ...
            btnHtml += `<button class="btn ${canAfford ? 'btn-warning' : 'btn-secondary'}" ${!canAfford ? 'disabled' : ''} style="width:100%; font-size:1.2rem; padding:20px;" onclick="document.getElementById('cardDetailModal').style.display='none'; window.buyShopItem('${cDef.id}', ${cDef.price}, ${isRes})">OSTA (${cDef.price} P)</button>`;
        }
    }
    
    btnHtml += `<button class="btn btn-secondary glass-card" style="width:100%; padding:15px; font-weight:bold; background:rgba(255,255,255,0.2); color:#fff; border:2px solid #fff;" onclick="document.getElementById('cardDetailModal').style.display='none'">❌ SULJE</button>`;
...
window.gmRemoveCurrentHole = function() {
    if(confirm("Haluatko poistaa nykyisen väylän?")) {
        ...
        update(ref(db), window.cleanFirebaseData({ 'gameState/currentHoleIndex': nextH, 'gameState/history': nextHistory, 'gameState/activeHole': nextActiveHole })); el('settingsModal').style.display='none';
    }
};

// PELAAJAN LISÄÄMINEN JA PISTEIDEN MUOKKAUS (Korjattu asettelu)
...
onValue(ref(db, 'gameState'), (snap) => {
    ...
    if (myName && currentCourse) {
        ...
        if (me) {
            ...
            el('myResPointsBtn').innerText = `${me.score || 0} P`; el('shopModalWallet').innerText = `${me.score || 0} P`; el('handCountBadge').innerText = `${myCards.length} / ${window.gameSettings.handLimit||6}`; 
        }
    }
});
