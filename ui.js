...
// ==============================================
// SKAALAUS (Kevyt varmuusskaalaus kapeille näytöille)
// ==============================================
window.applyViewScales = function() {
    let w = window.innerWidth;
    
    // Kauppa on max 550px leveä. Jos ruutu on pienempi, skaalataan hieman.
    let shopWrapper = el('board-shop-wrapper');
    if(shopWrapper) {
        let shopScale = w < 580 ? Math.max(0.6, w / 580) : 1;
        shopWrapper.style.transform = `scale(${shopScale})`;
        shopWrapper.style.transformOrigin = 'top center';
    }
    
    // Kansio on max 600px leveä
    let binderWrapper = el('board-binder-wrapper');
    if(binderWrapper) {
        let binderScale = w < 630 ? Math.max(0.6, w / 630) : 1;
...
window.showEventCard = function(cId, target, by) {
    ...
    if(actionArea) actionArea.innerHTML = targetStr;
...
window.openTargetModal = function(cId) {
    ...
    if (opponents.length === 1) {
        // Vain 1 vastustaja -> Iske automaattisesti häneen
        window.executeCardPlay(opponents[0].name);
        return;
    } else if (opponents.length > 1) {
        // Useampi vastustaja -> Pakotettu valinta
...
window.renderHoleView = function(hIndex, isCurrent) {
    ...
    if (hData.rule) {
        ...
        html += `
        <div class="post-it-note btn-modern" style="background:${bgCol}; transform: rotate(${rot2}deg); margin-bottom: 25px;" onclick="event.stopPropagation(); window.showZoomModal(this.outerHTML)">
            <div style="font-weight:900; font-size:0.85rem; margin-bottom:6px; text-transform:uppercase; color:#666;">📌 ${bTxt}</div>
            <div style="font-size:1.5rem; margin-bottom: 6px; font-weight: 900; line-height: 1.1; color:#111;">${hData.rule.n}</div>
            <div style="font-size:1.0rem; line-height: 1.35; font-weight:800; color:#222;">${hData.rule.d}</div>
        </div>`;
    }
    ...
    if(playedCards.length > 0) {
        ...
        playedCards.forEach((pc, idx) => {
            if (pc.target === window.myName || pc.target === 'KAIKKI VASTUSTAJAT') {
                ...
                html += `
                <div class="btn-modern" style="transform: rotate(${cRot}deg); cursor:pointer; width:140px; position:relative; overflow:hidden; box-shadow: 0 5px 15px rgba(0,0,0,0.5);" onclick="event.stopPropagation(); window.showEventCard('${pc.cardId}', '${encodedTarget}', '${encodedBy}')">
                    <div class="pushpin" style="left: ${pinLeft}%; z-index:20;"></div>
                    <div style="transform: scale(0.63); transform-origin: top left; width: 220px; height: 330px;">
                        ${fullCardHtml}
                    </div>
                </div>`;
            }
    ...
    <div class="clipboard-board">
        ...
        <div class="board-receipt-paper">
            <h2 style="color:var(--ink-blue); font-family: 'Kalam', cursive; font-size:1.8rem; text-align:center; border-bottom:3px solid var(--ink-blue); padding-bottom:5px; margin-bottom:15px; margin-top:0;">🏆 TULOS</h2>`;
    ...
    sortedPlayers.forEach((p) => {
        ...
        <div style="display:flex; justify-content:space-between; align-items:center; border-bottom:1px dashed #ccc; padding-bottom:8px; margin-bottom:8px;">
            <span style="font-size:1.3rem; font-family:'Kalam', cursive; color:var(--ink-blue); font-weight:bold;">${p.name}</span>
            <div style="display:flex; align-items:center; gap: 8px;">
                <span style="font-size:1rem; color:var(--warning); font-weight:900; background:#111; padding:3px 6px; border-radius:4px; box-shadow:inset 0 0 4px #000;">${displayScore}</span>
                <div class="score-display-paper" style="width:32px; height:32px; font-size:1.1rem; padding:0;">${scoreHTML}</div>
            </div>
        </div>`;
    });
    html += `</div></div>`;
    ...
    if (!isCurrent && window.myName && hData.pointBreakdowns && hData.pointBreakdowns[window.myName]) {
        ...
        if (myBreakdown.summary && myBreakdown.summary !== "Ei tuloja tai menoja.") {
            myBreakdown.summary.split(', ').forEach(part => {
                let kv = part.split(': ');
                if(kv.length === 2) { 
                    rowsHtml += `<tr><td style="padding:4px 0; color:#475569; text-align:left; font-weight:700;">${kv[0]}</td><td style="padding:4px 0; font-weight:900; color:#1e293b; text-align:right;">${kv[1]}</td></tr>`; 
                } else { 
                    rowsHtml += `<tr><td colspan="2" style="padding:4px 0; color:#475569; font-weight:bold; text-align:left;">${part}</td></tr>`; 
                }
            });
        } else {
             rowsHtml += `<tr><td colspan="2" style="padding:10px 0; color:#475569; text-align:center; font-style:italic;">Ei tapahtumia.</td></tr>`; 
        }
            ...
            <div class="wallet-stitching">
                <div class="wallet-receipt-paper">
                    <div style="border-bottom: 2px solid #000; padding-bottom: 8px; margin-bottom: 12px; text-align:center;">
                        <div style="font-weight:900; font-size:1.3rem; color:#1e293b; letter-spacing:1px;">PALKKAKUITTI</div>
                        <div style="font-size:0.8rem; color:#555; margin-top:4px;">Väylä ${hIndex} &nbsp;|&nbsp; Hljo: ${window.myName.toUpperCase()}</div>
                    </div>
                    <table style="width:100%; font-size:1rem; border-collapse: collapse; margin-bottom: 12px;">
                        ${rowsHtml}
                    </table>
                    <div style="border-top: 2px dashed #000; padding-top: 12px; display:flex; justify-content:space-between; align-items:center;">
                        <span style="font-size:1.1rem; font-weight:900; color:#1e293b;">NETTO</span>
                        <span style="font-size:1.3rem; font-weight:900; background:${bgDeltaColor}; padding:4px 8px; border:2px solid ${deltaColor}; border-radius:4px; color:${deltaColor};">${sign}${myBreakdown.delta} P</span>
                    </div>
                </div>
...
window.renderPayslipView = function(hIndex) {
    ...
    if (myBreakdown.summary && myBreakdown.summary !== "Ei tuloja tai menoja.") {
        myBreakdown.summary.split(', ').forEach(part => {
            let kv = part.split(': ');
            if(kv.length === 2) { 
                rowsHtml += `<tr><td style="padding:8px 0; color:#475569; text-align:left; font-weight:700;">${kv[0]}</td><td style="padding:8px 0; font-weight:900; color:#1e293b; text-align:right;">${kv[1]}</td></tr>`; 
            } else { 
                rowsHtml += `<tr><td colspan="2" style="padding:8px 0; color:#475569; font-weight:bold; text-align:left;">${part}</td></tr>`; 
            }
        });
    }
    ...
    <div class="leather-wallet">
        <div class="wallet-stitching">
            <div class="wallet-receipt-paper">
                <div style="border-bottom: 3px solid #000; padding-bottom: 12px; margin-bottom: 20px; text-align:center;">
                    <div style="font-weight:900; font-size:2rem; color:#1e293b; letter-spacing:2px; text-transform:uppercase;">Palkkakuitti</div>
                    <div style="font-size:1rem; color:#555; margin-top:6px; font-weight:bold;">Kausi: Väylä ${viewIndex}</div>
                    <div style="font-size:1.1rem; color:#1e293b; margin-top:10px;">Työntekijä: <b>${window.myName.toUpperCase()}</b></div>
                </div>
                <table style="width:100%; font-size:1.2rem; border-collapse: collapse; margin-bottom: 20px;">
                    ${rowsHtml}
                </table>
                <div style="border-top: 3px dashed #000; padding-top: 15px; display:flex; justify-content:space-between; align-items:center;">
                    <span style="font-size:1.4rem; font-weight:900; color:#1e293b;">NETTOPALKKA</span>
                    <span style="font-size:1.8rem; font-weight:900; background:${bgDeltaColor}; padding:6px 12px; border:3px solid ${deltaColor}; border-radius:6px; color:${deltaColor};">${sign}${myBreakdown.delta} P</span>
                </div>
            </div>
            <div class="wallet-pocket-front"></div>
        </div>
...
// ==============================================
// KANSIO (OMAT KORTIT) - JÄRKEVÄ RESPO-KOKO
// ==============================================
window.renderBinderOnBoard = function() {
    ...
    if(myCards.length === 0) {
        ...
        myCards.forEach((cId) => {
            ...
            let extraHtml = `<div style="text-align:center; font-weight:900; font-size:1rem; color:#111; margin-top:auto; padding-top:5px;">🔄 KATSO TASOT</div>`;
            ...
            if (cDef.nextId) {
                ...
                upgBtn = `<button class="btn ${canUpg ? 'btn-warning' : 'btn-secondary'} btn-modern" ${!canUpg ? 'disabled' : ''} style="width:100%; font-size:1rem; padding:10px; margin-bottom:8px; color:#000; font-weight:900;" onclick="event.stopPropagation(); window.upgradeCard('${cId}')">🔼 UPGRADE (${upgCost} P)</button>`;
            }

            cardsHtml += `
            <div style="display:flex; flex-direction:column; align-items:center; width:100%;">
                <div class="btn-modern" style="cursor:pointer; width:100%; margin-bottom:12px; position:relative; box-shadow:0 8px 20px rgba(0,0,0,0.5);" onclick="window.openCardDetail('${cId}', 'sell')">
                    ${fullCardHtml}
                </div>
                <div style="width:100%; max-width: 220px; z-index:20;">
                    ${upgBtn}
                    <button class="btn ${canPlay ? 'btn-success' : 'btn-secondary'} btn-modern" ${!canPlay ? 'disabled' : ''} style="width:100%; font-size:1.1rem; padding:12px; font-weight:900; margin-bottom:8px;" onclick="event.stopPropagation(); window.openTargetModal('${cDef.id}')">PELAA (${playCost} P)</button>
                    <button class="btn btn-danger btn-modern" style="width:100%; font-size:1rem; padding:10px; font-weight:bold; margin-bottom:5px;" onclick="event.stopPropagation(); window.forceDiscard('${cDef.id}')">♻️ MYY (+${sellReward} P)</button>
                </div>
            </div>`;
        });
    }
...
// ==============================================
// VÄLIPALA-AUTOMAATTI (KAUPPA) - Respo
// ==============================================
window.renderShopOnBoard = function() {
    ...
    levels.forEach(lvl => {
        ...
        shelvesHtml += `<div style="position:absolute; bottom:20px; left:0; width:100%; height:30px; background:repeating-linear-gradient(90deg, transparent, transparent 20px, #64748b 20px, #64748b 28px); z-index:1; opacity:0.9; filter:drop-shadow(0 10px 10px #000);"></div>`;
        ...
        for(let i=0; i<2; i++) {
            ...
            if (item) {
                ...
                let extraHtml = `<div style="text-align:center; font-weight:900; font-size:1rem; color:#111; margin-top:auto; padding-top:5px;">🔄 TARKASTELE</div>`;
                ...
                shelvesHtml += `
                    <div style="position:relative; width:100%; max-width:220px; display:flex; flex-direction:column; align-items:center; z-index:10;">
                        <div class="btn-modern" style="cursor:pointer; width:100%; margin-bottom:10px; box-shadow:0 8px 15px rgba(0,0,0,0.6);" onclick="window.openCardDetail('${item.id}', 'shop')">
                            ${fullCardHtml}
                        </div>
    ...
    if(actRes.length > 0) {
        ...
        actRes.forEach(rId => {
            ...
            const canAfford = myPoints >= itemPrice;
            let extraHtml = `<div style="text-align:center; font-weight:900; font-size:0.9rem; color:#111; margin-top:auto; padding-top:5px;">🔄 TARKASTELE</div>`;
            let fullCardHtml = window.generateCardHTML(resItem, false, extraHtml, false);
            
            reserveHtml += `
                <div style="width:48%; max-width:220px; display:flex; flex-direction:column; align-items:center;">
                    <div class="btn-modern" style="width:100%; cursor:pointer; position:relative; margin-bottom:10px;" onclick="window.openCardDetail('${resItem.id}', 'shop_res')">
    ...
    <div class="shop-machine">
        <div style="background: linear-gradient(135deg, #000, #111); padding: 15px; border-radius: 10px; border: 4px solid #222; display:flex; justify-content:space-between; align-items:center; margin-bottom: 20px; box-shadow: inset 0 0 20px rgba(239,68,68,0.25);">
            <div style="display:flex; align-items:center; gap: 15px;">
                <div style="font-size:2.5rem; text-shadow: 0 0 10px rgba(255,255,255,0.6);">🥨</div>
                <div style="color:#ef4444; font-family:'Courier Prime', monospace; font-weight:900; font-size:2rem; letter-spacing: 2px; text-shadow: 0 0 10px #ef4444;">SNACKS</div>
            </div>
            
            <div style="background: #000; padding: 8px 15px; border-radius: 6px; border: 2px solid #222; box-shadow: inset 0 0 10px #000, 0 0 8px rgba(34,197,94,0.3); display: flex; flex-direction: column; align-items: center;">
                <span style="color: #444; font-size: 0.65rem; font-family: 'Courier Prime', monospace; font-weight: 900; letter-spacing: 1px; margin-bottom: 3px;">CREDIT</span>
                <span style="color:#22c55e; font-family:'Courier Prime', monospace; font-size:2rem; font-weight:900; text-shadow: 0 0 10px #22c55e;">${myPoints} P</span>
            </div>
        </div>
        ...
        <div style="padding: 20px 0; display:flex; gap:15px; align-items:center;">
            <div style="width: 120px; background: #000; border-radius: 8px; border: 4px solid #1e293b; padding: 10px; display:flex; flex-direction:column; align-items:center; box-shadow: inset 0 0 10px rgba(0,0,0,0.9);">
                <div style="width: 100%; height: 25px; background: #0f172a; border: 2px solid #334155; margin-bottom: 10px; color:#22c55e; font-family:'Courier Prime', monospace; font-weight:bold; font-size:1rem; text-align:right; padding-right:5px; line-height:21px;">12.00</div>
                <div style="display:grid; grid-template-columns: repeat(3, 1fr); gap: 6px; width:100%;">
                    ${Array(9).fill('<div style="background: #334155; height:15px; border-radius:3px; border-bottom:3px solid #0f172a;"></div>').join('')}
                </div>
            </div>
            ...
            <div style="flex: 1; background: #000; border-radius: 8px; border: 6px solid #111; position:relative; box-shadow: inset 0 20px 30px #000;">
                <div style="position:absolute; top:0; left:0; right:0; height: 60px; background: #1a1a1a; border-bottom: 3px solid #000; display:flex; justify-content:center; align-items:center; transform-origin: top; transform: rotateX(-15deg);">
                    <span style="color:#444; font-weight:900; font-size:1.8rem; letter-spacing:8px; text-shadow:-1px -1px 0 #000;">PUSH</span>
                </div>
    ...
    window.allCards.forEach(c => {
        ...
        <div style="display:flex; justify-content:space-between; align-items:center; background:#f8fafc; padding:10px; border-radius:8px; border:2px solid #cbd5e1;">
            ...
            <div style="display:flex; align-items:center; gap: 6px;">
                ...
                <input type="number" id="price_${c.id}" value="${currentPrice}" style="width:60px; padding:6px; border-radius:6px; border:2px solid #94a3b8; text-align:center; font-weight:900; font-size:1rem;">
            </div>
        </div>`;
    });
    container.innerHTML = html;
...
window.initNativeCarousel = function() {
    ...
    container.addEventListener('scroll', () => { 
        requestAnimationFrame(() => {
            ...
            // Oletetaan jättikortin leveys + marginaalit
            const cardWidth = 340; 
            ...
            for (let i = 0; i < cards.length; i++) {
                ...
                card.style.transform = `translate3d(${diff * -30}px, ${Math.abs(diff) * 15}px, ${Math.abs(diff) * -120}px) rotateZ(${diff * 4}deg) scale(${Math.max(0.85, 1 - Math.abs(diff) * 0.15)})`;
