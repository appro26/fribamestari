...
window.generateCardHTML = function(cDef, isLocked = false, extraBottomHtml = '', isCarousel = false) {
    ...
    // Karusellissa kortti skaalautuu 100%, taululla fyysinen 175px leveys
...
window.renderShopOnBoard = function() {
            ...
            if (item) {
                ...
                shelvesHtml += `
                    <div style="position:relative; width:45%; display:flex; flex-direction:column; align-items:center;">
                        <div style="transform: scale(1.5); cursor:pointer; width:175px; margin-bottom: 90px; transform-origin: top center;" onclick="window.openCardDetail('${item.id}', 'shop')">
                            ${fullCardHtml}
                        </div>
                ...
                shelvesHtml += `
                    <div style="position:relative; width:45%; display:flex; flex-direction:column; align-items:center;">
                        <div style="width:175px; height:260px; display:flex; align-items:center; justify-content:center; background:rgba(0,0,0,0.4); border-radius:8px; border:4px dashed #333; transform: scale(1.5); margin-bottom: 90px; transform-origin: top center;">
    ...
    // Leveys tasan 650px. Kun zoomaus iskee tähän, puhelin on täynnä suuria tekstejä!
    wrapper.innerHTML = `
...
// ==============================================
// TÄMÄ ON GPU:N PELASTAJA (RENDERBOARD UUSIKSI)
// ==============================================
window.renderBoard = function() {
    const board = el('corkboard-surface');
    if (!board || !window.currentCourse) return;
    
    // 1. SIIRRETÄÄN ILMOITUSTAULUN RASKAS TAUSTA SUORAAN BODY-ELEMENTTIIN!
    // Näin puhelimen ei tarvitse koskaan liikuttaa taustaa, ja pirstaloituminen on mahdotonta.
    document.body.style.cssText = "margin: 0; padding: 0; width: 100vw; height: 100vh; overflow: hidden; background-color: #e2e8f0; background-image: radial-gradient(rgba(0,0,0,0.08) 3px, transparent 3px); background-size: 15px 15px; touch-action: none; overscroll-behavior: none;";
    
    // 2. ITSE LIIKKUVA LAUTA ON NYT KOOLTAAN 0x0 PIKSELIÄ!
    ...
    // Lisätään irtonaiset elementit "tyhjään" ankkuripisteeseen
    ...
    let shopX = startXHolesVal + (cols * 380) + ((cols - 1) * 80) + 150;
    let shopY = 200; 
...
// ==============================================
// KARUSELLI JA MUUT TOIMINNOT (Säilytetty ennallaan)
// ==============================================
...
window.renderCarouselActionButtons = function() {
    ...
    // UI napit myyntiin, upgreidiin, ostoon (Säilytetty täysin identtisenä)
    // Tässä on tilaa sulkemisnapille joka piirretään index.html:n puolelta!
    if (mode === 'sell') {
        let cDef = window.allCards.find(c => c && c.id === cId);
        if(cDef) {
...
window.openCardDetail = function(cId, mode) {
    ...
    // TÄRKEÄÄ: Pakotetaan karusellin modaalille UI tyylikorjaus lennossa, jos styleFixiä ei ole tehty
    let mod = el('cardDetailModal');
    if(mod) {
        mod.style.paddingBottom = "120px";
        mod.style.justifyContent = "flex-start";
        mod.style.paddingTop = "5vh";
    }
