...
window.applyViewScales = function() {
    ...
    // Poistettu pakotetut globaalit skaalaukset.
    // CSS Grid ja flexbox hoitavat asettelun paljon luotettavammin.
    // Varmistetaan vain, että tulokset mahtuvat kaikkein pienimmille ruuduille.
...
window.renderHoleView = function(hIndex, isCurrent) {
    ...
    if (isCurrent && hData.penColor) {
        // TÄSSÄ KORJATTU KYNÄ: Poistettu .btn-modern, asennettu puhdas pen-container
        html += `
        <div class="pen-container" style="cursor:pointer;" onclick="event.stopPropagation(); window.openScoreModal();">
    ...
    // HUOM: Tuloslappua tai palkkakuittia ei enää piirretä väylänäkymään, 
    // jotta alue pysyy siistinä ja puhtaana. Ne löytyvät omilta välilehdiltään!
...
window.renderPayslipView = function(hIndex) {
    ...
    if (myBreakdown.summary && myBreakdown.summary !== "Ei tuloja tai menoja.") {
        myBreakdown.summary.split(', ').forEach(part => {
            let kv = part.split(': ');
            if(kv.length === 2) { 
                rowsHtml += `<tr><td style="padding:8px 0; color:#333; text-align:left;">${kv[0]}</td><td style="padding:8px 0; font-weight:900; color:#000; text-align:right;">${kv[1]}</td></tr>`; 
            } else { 
                rowsHtml += `<tr><td colspan="2" style="padding:8px 0; color:#333; font-weight:bold; text-align:left;">${part}</td></tr>`; 
            }
        });
    }

    // AITO SUOMALAINEN PALKKALASKELMA
    container.innerHTML = `
    <div class="payslip-paper-finnish">
        <div class="payslip-header">
            <h2>Palkkalaskelma</h2>
...
window.renderBinderOnBoard = function() {
    ...
    if(myCards.length === 0) {
        ...
        myCards.forEach((cId) => {
            ...
            // Jättimäiset kortit gridiin, ei nappeja
            cardsHtml += `
            <div class="btn-modern" style="cursor:pointer; width:100%; position:relative; box-shadow:0 10px 25px rgba(0,0,0,0.5); transition:transform 0.1s;" onclick="window.openCardDetail('${cId}', 'sell')">
                ${fullCardHtml}
            </div>`;
        });
    }
    ...
    levels.forEach(lvl => {
        ...
        for(let i=0; i<2; i++) {
            ...
            if (item) {
                ...
                // Isot kortit, napit ja hinta omassa blokissaan
                shelvesHtml += `
